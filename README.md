...
基于大模型（LLM）的测试用例自动生成工具。输入需求文档或 UI 截图，自动生成高质量测试用例，支持 Excel 和 Markdown 双格式导出，提供 Web 可视化界面。
...
| 导航项 | 功能 |
|--------|------|
| 需求编写 | 输入需求、上传文件、生成用例、AI 评审、优化、导出、偏好管理 |
| 模型配置 | 预设模型快速切换（DeepSeek / MiMo / 通义千问），自定义生成和评审模型参数 |
...
| 历史记录 | 点击 Header 的"历史"按钮，查看当前用户的所有生成记录，支持加载、删除 |
...
| 区域 | 功能 |
|------|------|
| 预设模型 | 一键切换 DeepSeek / MiMo / 通义千问，自动填充配置 |
| 生成模型 | 配置用于生成用例的模型：Base URL、模型名、API Key、Temperature、图片模型、思考模式 |
| 评审模型 | 配置独立评审模型（可选），不启用则复用生成模型 |
...
testcase-gen/
├── start.py             # Web 版启动器
├── web.py               # Flask Web 应用（SSE 流式 API + 并行生成 + 认证 + 模型配置）
├── templates/
│   └── index.html       # Web 前端页面（左侧导航 + 需求编写 + 模型配置）
├── main.py              # 命令行入口
...
├── db.py                # SQLite 数据库（用户 + 历史记录 + 偏好规则 + 设置）
...
> 模型配置也可以在 Web 界面的「模型配置」页面修改，会覆盖 config.yaml 中的对应配置。
...
A: 检查 `config.yaml` 或 Web 界面「模型配置」中的 API Key 是否正确，确认模型服务是否到期。
...
**Q: 生成的用例太多 / 太少**
A: 在配置中调整 `max_testcases`（默认 100）。超出上限时自动按优先级保留高优先级用例。如用例太少，可在需求中更详细地描述各功能模块。

**Q: 优化后用例数量减少了**
A: 这是正常的去重结果。评审阶段会检测重复用例并输出清单，优化阶段按清单精准删除重复项，同时补充遗漏场景。如果优化结果为空会自动回退到原始用例。

**Q: 图片上传后 AI 没有识别到界面元素**
A: 确认配置了 `image_model`（多模态模型）。图片会贯穿整个生成链路：模块分析时结合图片拆解模块，每个模块生成时结合图片中的具体元素编写用例。

**Q: 如何切换模型**
A: 在 Web 界面点击左侧「模型配置」，可一键切换预设模型（DeepSeek / MiMo / 通义千问），也可手动修改各项参数。命令行版需编辑 `config.yaml`。

**Q: 思考模式是什么**
A: 开启后 LLM 会先做深度推理再输出，对复杂需求的场景覆盖更全面，但响应更慢、token 消耗更多。在模型配置中设置 `enable_thinking` 开启。

**Q: Web 版端口被占用**
A: 使用 `python start.py -p 8080` 指定其他端口。

**Q: 历史记录保存在哪里**
A: 保存在项目目录下的 `data.db`（SQLite 数据库）。删除该文件会清空所有历史记录、偏好规则和用户账号。
...
**Q: 忘记密码怎么办**
A: 当前版本不支持密码找回。可直接注册新账号，或删除 `data.db` 中对应用户的记录重置。

...
| 1️⃣1️⃣ | `docs/v2/step11-benchmark-evaluation.md` | ~440 | **Step 11 冻结设计 v1.0**（架构边界 18 条 / Gold 独立性 / 四态 Matching / Metrics 手册 / 双轨评价 / Reliability 五态 / bench-v0.1 数据契约）+ **附录 B：S6 Runner 实施说明**（源 3 阶段门控 / S6 冻结决策 / 指纹与落盘安全） | 要动 Benchmark/评价层（S7~S8）前**必读** |
...
**当前进度（截至 2026-09-21）：Step 1~10 全部完成并推送（含 `cdb325c` V2 产品级 UI/UX 重构——React+TS+Vite `frontend-v2/`，产物 `static/v2/v2_react.js`+`v2_react.css`，Run-centric IA，只读端点 requirements 13→14，旧 `v2_app.js`/`v2_style.css` 退役，详见 `docs/v2/v2-ui-react-refactor.md`——与 `d1c5345` V2 Product Polish）。中后期路线评审已由用户拍板：Step 11 = Quality Evaluation Foundation + Benchmark Foundation，冻结设计见 `docs/v2/step11-benchmark-evaluation.md`。Step 11 已完成 S1~S6：设计冻结 + eval schema/loader + 确定性评价引擎 matching/metrics_hard + **bench-v0.1 正式数据集**（10 case / 10 requirement / 83 人工独立 Gold / manifest）+ S5 语义评价层 + **S6 Benchmark Runner**（`scripts/v2_benchmark.py` 独立进程 + 独立 benchmark DB + 五态启发式分类 + 环境指纹 + runset 快照）。S1~S4 已推（`0cb4bf5`）、S5 已推（`7b59bfe`）、S6 本地待推。全量 **1223 passed + 3 skipped**，ruff 双绿，schema_version=10，V1 零改动。下一步 = S7 真实 runset 执行 + Baseline v0.1（待用户指令，见 §11）。**
...
现有 V1
 ...
 │          S5 ✅ / S6 Runner ✅ / S7~S8（真实 runset+Baseline / Compare）⬜ 未开始
...
| 11 Quality Eval + Benchmark（S1~S8） | ⏳ **S1~S6 ✅ / S7~S8 ⬜** | S1 `docs/v2/step11-benchmark-evaluation.md`（冻结 v1.0）；S2 `core/v2/eval/schema.py`（Case/Gold/Manifest+loader）；S3 `core/v2/eval/`{matching,metrics_hard}.py（四态匹配+硬指标+RunObservation 契约，零 LLM）；S4 `benchmark/`（bench-v0.1：10 case+10 requirement+11 gold+manifest）+ 4 测试文件 147 例；S5 `core/v2/eval/`{metrics_soft,semantic_prompts}.py（语义三态+forbidden suspect+candidate 预审+JSON 契约）+ 34 例；S6 `core/v2/eval/runner_lib.py`+`scripts/v2_benchmark.py`（独立 CLI 进程+独立 benchmark DB+五态阶段门控分类+环境指纹+runset 快照+生产库硬护栏）+ 58 例；详见 §11 | S1~S4 已推 (`0cb4bf5`)；S5 已推 (`7b59bfe`)；S6 本地待推 |
| 12~13 | ⬜ 未开始（Step 12 剩余主题与 Step 13 待 S8 后再评审） | — | — |

**测试基线**：V1 原有 126 例（零回归）+ V2 新增，**当前全量 1223 passed + 3 skipped**（Step 11 S2~S6 新增 239 例 Benchmark/Eval 测试，其中 S5 语义层 34、S6 Runner 58；启用 V2_RUN_REAL_LLM=1 后 +3），schema_version=10，ruff check/format 全绿。
...
core/v2/         # V2 持久层 + 领域服务（独立 data_v2.db，schema_version=10）
  ...
  eval/        # Step 11 Benchmark 评价层（Runtime 外部只读观测，零 DB 写入）
    ...
    metrics_soft.py semantic_prompts.py # S5：语义三态 + forbidden suspect + candidate 预审 + benchmark-soft-v1 JSON 契约
...
.\.venv\Scripts\python.exe -m pytest -q                    # 期望 1223 passed + 3 skipped（real_llm marker 默认 skip；启用 V2_RUN_REAL_LLM=1 后 1226）
...
## 11. Step 11「Quality Evaluation + Benchmark Foundation」（S1~S6 ✅ 2026-09-21；S1~S4 已推 `0cb4bf5`，S5 已推 `7b59bfe`，S6 本地待推）

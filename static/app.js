...
function enterApp(u){document.getElementById('authPage').style.display='none';document.getElementById('appPage').style.display='block';document.getElementById('headerUsername').textContent=u.username;document.getElementById('dashGreeting').textContent=u.username+'，你好';loadDashboard();checkDraft();}
...
let csrfToken='';
// Ctrl+Z 撤销
document.addEventListener('keydown',e=>{if((e.ctrlKey||e.metaKey)&&e.key==='z'&&!e.shiftKey){const ae=document.activeElement;if(ae&&(ae.tagName==='INPUT'||ae.tagName==='TEXTAREA'))return;e.preventDefault();undo();}});
function pushUndo(index,before,action){undoStack.push({action,index,before});if(undoStack.length>20)undoStack.shift();}
function undo(){if(!undoStack.length){showToast('没有可撤销的操作');return;}const op=undoStack.pop();if(op.action==='edit'){currentTestcases[op.index]=op.before;}else if(op.action==='batchDelete'){currentTestcases=op.snapshot;}else if(op.action==='batchPriority'){currentTestcases=op.snapshot;}renderResult({testcases:currentTestcases,count:currentTestcases.length,files:currentFiles,session_id:currentSessionId,input:{}});saveDraft();showToast('已撤销');}
// 自动保存草稿
const DRAFT_KEY='tc_gen_draft';
function saveDraft(){if(!currentTestcases.length)return;try{localStorage.setItem(DRAFT_KEY,JSON.stringify({testcases:currentTestcases,requirement:currentRequirement,sessionId:currentSessionId,savedAt:Date.now()}));}catch(e){}}
function clearDraft(){localStorage.removeItem(DRAFT_KEY);}
function checkDraft(){try{const raw=localStorage.getItem(DRAFT_KEY);if(!raw)return;const draft=JSON.parse(raw);if(Date.now()-draft.savedAt>86400000){clearDraft();return;}if(confirm('发现上次未保存的用例修改（'+new Date(draft.savedAt).toLocaleString()+'），是否恢复？')){currentTestcases=draft.testcases||[];currentRequirement=draft.requirement||'';currentSessionId=draft.sessionId||null;if(currentTestcases.length){const el=document.getElementById('requirement');if(el)el.value=currentRequirement;renderResult({testcases:currentTestcases,count:currentTestcases.length,files:{},session_id:currentSessionId,input:{}});showToast('已恢复 '+currentTestcases.length+' 条用例');}}else{clearDraft();}}catch(e){clearDraft();}}
...
let undoStack=[];
let sortField=null,sortAsc=true,filterText='',filterModule='',filterPriority='',renderedIndices=[];
...
async function doGenerate(useAnalysis){
    ...
    }catch(e){showToast(e.message,'error');}finally{isGenerating=false;btn.disabled=false;btn.innerHTML='&#9889; 直接生成';document.getElementById('loading').style.display='none';clearDraft();}
}
async function regenerateCase(i){const tc=currentTestcases[i];const btn=document.getElementById('regen-btn-'+i);if(btn){btn.disabled=true;btn.innerHTML='<span class="spinner"></span>';}pushUndo(i,JSON.parse(JSON.stringify(tc)),'edit');try{const resp=await fetch('/api/regenerate-single',{method:'POST',headers:_headers({'Content-Type':'application/json'}),body:JSON.stringify({requirement:currentRequirement,testcase:tc})});if(resp.status===401){doLogout();throw new Error('登录已过期');}const rd=resp.body.getReader(),dec=new TextDecoder();let buf='';while(true){const{done,value}=await rd.read();if(done)break;buf+=dec.decode(value,{stream:true});const lines=buf.split('\n');buf=lines.pop();for(const line of lines){if(!line.startsWith('data: '))continue;try{const ev=JSON.parse(line.slice(6));if(ev.type==='done'&&ev.data){currentTestcases[i]=ev.data;filterText='';document.getElementById('tcSearch').value='';renderResult({testcases:currentTestcases,count:currentTestcases.length,files:currentFiles,session_id:currentSessionId,input:{}});const row=document.getElementById('row-'+i);if(row)row.classList.add('row-highlight');saveDraft();showToast('已重新生成: '+ev.data.title);}else if(ev.type==='error')throw new Error(ev.message);}catch(e){if(e.message&&!e.message.includes('JSON'))throw e;}}}}catch(e){showToast('重新生成失败: '+e.message,'error');}finally{if(btn){btn.disabled=false;btn.innerHTML='AI';}}}
function renderResult(data){
    ...
    document.querySelectorAll('.sort-icon').forEach(el=>{el.className='sort-icon';});if(sortField){const icon=document.getElementById('sort-'+sortField);if(icon)icon.classList.add(sortAsc?'asc':'desc');}
    const modSel=document.getElementById('filterModule');if(modSel){const mods=[...new Set(tc.map(t=>t.module||'未分类'))].sort();const cv=filterModule;modSel.innerHTML='<option value="">全部模块</option>'+mods.map(m=>`<option value="${esc(m)}" ${m===cv?'selected':''}>${esc(m)}</option>`).join('');}
    document.getElementById('filterBar').style.display=tc.length?'flex':'none';
    applyFilters();
...
// ========== 搜索、筛选、排序 ==========
function applyFilters(){const q=(document.getElementById('tcSearch')?.value||'').toLowerCase();const mod=document.getElementById('filterModule')?.value||'';const pri=document.getElementById('filterPriority')?.value||'';filterText=q;filterModule=mod;filterPriority=pri;let indices=currentTestcases.map((_,i)=>i);if(q){indices=indices.filter(i=>{const t=currentTestcases[i];return(t.title||'').toLowerCase().includes(q)||(t.module||'').toLowerCase().includes(q)||(t.steps||'').toLowerCase().includes(q)||(t.expected||'').toLowerCase().includes(q);});}if(mod)indices=indices.filter(i=>(currentTestcases[i].module||'未分类')===mod);if(pri)indices=indices.filter(i=>currentTestcases[i].priority===pri);if(sortField){const PO={P0:0,P1:1,P2:2,P3:3};indices.sort((a,b)=>{const ta=currentTestcases[a],tb=currentTestcases[b];let va=ta[sortField]||'',vb=tb[sortField]||'';if(sortField==='priority'){va=PO[va]??9;vb=PO[vb]??9;}let cmp=String(va).localeCompare(String(vb),'zh');return sortAsc?cmp:-cmp;});}renderedIndices=indices;document.getElementById('filterCount').textContent=indices.length!==currentTestcases.length?`${indices.length}/${currentTestcases.length} 条`:'';renderTableRows(indices);}
function sortBy(field){if(sortField===field)sortAsc=!sortAsc;else{sortField=field;sortAsc=true;}document.querySelectorAll('.sort-icon').forEach(el=>{el.className='sort-icon';});if(sortField){const icon=document.getElementById('sort-'+sortField);if(icon)icon.classList.add(sortAsc?'asc':'desc');}applyFilters();}
function renderTableRows(indices){const tbody=document.getElementById('tbody');if(!indices.length&&currentTestcases.length){tbody.innerHTML='<tr><td colspan="8" style="text-align:center;color:#999;padding:20px;">没有匹配的用例</td></tr>';return;}tbody.innerHTML=indices.map(oi=>{const t=currentTestcases[oi];const cb=selectedRows.has(oi)?'checked':'';return`<tr id="row-${oi}"><td class="cb-cell"><input type="checkbox" class="row-cb" data-idx="${oi}" ${cb} onchange="toggleRowSelect(${oi})"></td><td><strong>${esc(t.id)}</strong></td><td>${esc(t.module)}</td><td>${esc(t.title)}</td><td><span class="priority priority-${esc(t.priority)}">${esc(t.priority)}</span></td><td><span class="type-tag">${esc(t.type)}</span></td><td><span class="src-tag" title="${esc(t.source||'')}">${esc(t.source||'-')}</span></td><td><button class="action-btn" onclick="toggleDetail(${oi})">详情</button> <button class="action-btn" onclick="editCase(${oi})">编辑</button> <button class="action-btn regen" id="regen-btn-${oi}" onclick="regenerateCase(${oi})">AI</button></td></tr><tr class="detail-row" id="detail-${oi}"><td colspan="8"><div class="detail-grid"><div><dt>前置条件</dt><dd>${esc(t.precondition)||'无'}</dd></div><div><dt>预期结果</dt><dd>${esc(t.expected)}</dd></div><div style="grid-column:1/-1"><dt>测试步骤</dt><dd>${esc(t.steps).replace(/\\n/g,'<br>')}</dd></div></div></td></tr>`;}).join('');}
function downloadFile(type){clearDraft();if(type==='json'){
...
function batchChangePriority(){const val=document.getElementById('batchPriority').value;if(!val||!selectedRows.size){showToast('请先选择用例和优先级','error');return;}const snapshot=JSON.parse(JSON.stringify(currentTestcases));const affected=[...selectedRows].map(i=>({index:i,before:JSON.parse(JSON.stringify(currentTestcases[i]))}));undoStack.push({action:'batchPriority',affected,snapshot});if(undoStack.length>20)undoStack.shift();selectedRows.forEach(i=>{currentTestcases[i].priority=val;});renderResult({testcases:currentTestcases,count:currentTestcases.length,files:currentFiles,session_id:currentSessionId,input:{}});saveDraft();showToast(`已将 ${selectedRows.size} 条用例改为 ${val}`);}
async function batchDelete(){if(!selectedRows.size){showToast('请先选择用例','error');return;}if(!await showConfirm(`确定删除 ${selectedRows.size} 条用例？`,{danger:true}))return;const snapshot=JSON.parse(JSON.stringify(currentTestcases));const deleted=[...selectedRows].sort((a,b)=>a-b).map(i=>({index:i,tc:JSON.parse(JSON.stringify(currentTestcases[i]))}));undoStack.push({action:'batchDelete',deleted,snapshot});if(undoStack.length>20)undoStack.shift();const sorted=[...selectedRows].sort((a,b)=>b-a);sorted.forEach(i=>currentTestcases.splice(i,1));currentTestcases.forEach((t,j)=>{t.id='TC_'+String(j+1).padStart(3,'0');});renderResult({testcases:currentTestcases,count:currentTestcases.length,files:currentFiles,session_id:currentSessionId,input:{}});saveDraft();showToast(`已删除 ${sorted.length} 条用例`);}
...
let allHistoryItems=[];
async function loadHistory(){const l=document.getElementById('historyList');try{const r=await authFetch('/api/history?limit=50');const d=await r.json();const ss=d.sessions||[];allHistoryItems=ss;if(!ss.length){l.innerHTML='<div class="history-empty">暂无历史记录</div>';return;}l.innerHTML=ss.map((s,idx)=>`<div class="history-item" data-search="${esc((s.requirement||'')+(s.requirement_preview||'')).toLowerCase()}"><div class="h-time">${esc(s.created_at)}</div><div class="h-preview" title="${esc(s.requirement||'')}">${esc(s.requirement_preview)}</div><div class="h-meta"><span>${s.tc_count} 条</span>${s.priority?'<span>'+esc(s.priority)+'</span>':''}</div><div class="h-actions"><button onclick="loadSession(${s.id})">加载</button><button class="del-btn" onclick="deleteSession(${s.id},event)">删除</button></div></div>`).join('');}catch(e){l.innerHTML='<div class="history-empty">加载失败</div>';}}
function filterHistory(){const q=(document.getElementById('historySearch')?.value||'').toLowerCase();document.querySelectorAll('.history-item').forEach(el=>{const text=el.getAttribute('data-search')||'';el.style.display=(!q||text.includes(q))?'':'none';});}
...
// 内联编辑
// ============================================================
function editCase(i){const tc=currentTestcases[i],row=document.getElementById('row-'+i),dr=document.getElementById('detail-'+i);row.innerHTML=`<td class="cb-cell"><input type="checkbox" class="row-cb" data-idx="${i}"></td><td><strong>${esc(tc.id)}</strong></td><td><input class="edit-input" id="edit-module-${i}" value="${esc(tc.module)}" style="width:75px;"></td><td><input class="edit-input" id="edit-title-${i}" value="${esc(tc.title)}"></td><td><select id="edit-priority-${i}" style="padding:2px;border:1px solid #1a73e8;border-radius:4px;font-size:11px;"><option value="P0" ${tc.priority==='P0'?'selected':''}>P0</option><option value="P1" ${tc.priority==='P1'?'selected':''}>P1</option><option value="P2" ${tc.priority==='P2'?'selected':''}>P2</option><option value="P3" ${tc.priority==='P3'?'selected':''}>P3</option></select></td><td><input class="edit-input" id="edit-type-${i}" value="${esc(tc.type)}" style="width:65px;font-size:11px;"></td><td><span class="src-tag">${esc(tc.source||'-')}</span></td><td class="edit-actions"><button class="edit-save" onclick="saveEdit(${i})">保存</button><button class="edit-cancel" onclick="cancelEdit(${i})">取消</button></td>`;dr.classList.add('show');const stepsArr=(tc.steps||'').split('\n').filter(s=>s.trim());const stepRows=stepsArr.map((s,si)=>`<div class="step-row"><span class="step-num">${si+1}.</span><input value="${esc(s.replace(/^\d+[.、]\s*/,''))}" placeholder="输入步骤${si+1}"><button class="step-del" onclick="this.parentElement.remove();editRenumberSteps(${i})" title="删除">&times;</button></div>`).join('');dr.innerHTML=`<td colspan="8" style="background:#f8fbff;"><div class="detail-grid"><div><dt>前置条件</dt><textarea class="edit-textarea" id="edit-precondition-${i}" rows="2" placeholder="如：用户已登录，进入XX页面">${esc(tc.precondition)}</textarea></div><div><dt>预期结果</dt><textarea class="edit-textarea" id="edit-expected-${i}" rows="2" placeholder="具体的系统响应，如：页面跳转到...">${esc(tc.expected)}</textarea></div><div style="grid-column:1/-1"><dt>测试步骤</dt><div class="step-edit-list" id="stepList-${i}">${stepRows}</div><button class="step-add-btn" onclick="editAddStep(${i})">+ 添加步骤</button></div></div></td>`;}
function editAddStep(i){const list=document.getElementById('stepList-'+i);const n=list.children.length+1;const div=document.createElement('div');div.className='step-row';div.innerHTML=`<span class="step-num">${n}.</span><input value="" placeholder="输入步骤${n}"><button class="step-del" onclick="this.parentElement.remove();editRenumberSteps(${i})" title="删除">&times;</button>`;list.appendChild(div);div.querySelector('input').focus();}
function editRenumberSteps(i){const list=document.getElementById('stepList-'+i);if(!list)return;list.querySelectorAll('.step-row').forEach((row,idx)=>{row.querySelector('.step-num').textContent=(idx+1)+'.';row.querySelector('input').placeholder='输入步骤'+(idx+1);});}
function saveEdit(i){const tc=currentTestcases[i];if(!originalTestcases.length)originalTestcases=JSON.parse(JSON.stringify(currentTestcases));pushUndo(i,JSON.parse(JSON.stringify(tc)),'edit');tc.module=document.getElementById('edit-module-'+i).value;tc.title=document.getElementById('edit-title-'+i).value;tc.priority=document.getElementById('edit-priority-'+i).value;tc.type=document.getElementById('edit-type-'+i).value;tc.precondition=document.getElementById('edit-precondition-'+i).value;tc.expected=document.getElementById('edit-expected-'+i).value;const stepInputs=document.querySelectorAll('#detail-'+i+' .step-row input');if(stepInputs.length){const steps=[];stepInputs.forEach(inp=>{const v=inp.value.trim();if(v)steps.push(v);});tc.steps=steps.join('\n');}else{tc.steps=document.getElementById('edit-steps-'+i).value;}renderResult({testcases:currentTestcases,count:currentTestcases.length,files:currentFiles,session_id:currentSessionId,input:{}});document.getElementById('btnSavePref').style.display='';saveDraft();showToast('已保存');}
